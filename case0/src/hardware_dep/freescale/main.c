...
static void process_packet(packet_descriptor_t *pkt, lookup_table_t **table);
...
static int pktio_queue_thread(void *arg) {
    ...
    tables[TABLE_smac] = malloc(sizeof(lookup_table_t));
    tables[TABLE_dmac] = malloc(sizeof(lookup_table_t));
    exact_create(tables[TABLE_smac], 1);
    exact_create(tables[TABLE_dmac], 1);
    uint8_t *smac_value = malloc(sizeof(struct smac_action));
    uint8_t *dmac_value = malloc(sizeof(struct dmac_action));
    ((struct smac_action *) smac_value)->action_id = action__nop;
    ((struct dmac_action *) dmac_value)->action_id = action_forward;
    uint8_t *port = malloc(sizeof(uint8_t));
    *port = 1;
    memcpy(((struct dmac_action *) dmac_value)->forward_params.port, port, 2);

    int thr;
    odp_pktio_t pktio, pktio2;
    thread_args_t *thr_args;
    odp_pktout_queue_t pktout, pktout0, pktout1;
    odp_queue_t inq;
    odp_packet_t pkt;
    odp_event_t ev;
...
static void process_packet(packet_descriptor_t *pkt, lookup_table_t **tables) {
    handle_packet(pkt, tables);
}
