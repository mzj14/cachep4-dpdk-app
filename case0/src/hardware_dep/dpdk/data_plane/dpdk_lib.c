...
create_table(lookup_table_t *t, int socketid) {
    t->socketid = socketid;
    if (t->key_size == 0)
        return; // we don't create the table if there are no keys (it's a fake table for an element in the pipeline)
    switch (t->type) {
        case LOOKUP_EXACT:
...
create_counters_on_socket(int socketid) {
    ...
    for (lcore_id = 0; lcore_id < RTE_MAX_LCORE; lcore_id++) {
        ...
        for (i = 0; i < NB_COUNTERS; i++) {
            ...
            state[socketid].counters[i][lcore_id]->values = (vector_t *) rte_malloc_socket("vector_t", sizeof(vector_t),
                                                                                           0, socketid);
            vector_init(state[socketid].counters[i][lcore_id]->values, 1 /* initial size */, c.size,
                        sizeof(rte_atomic32_t), (void (*)(void *)) &rte_atomic32_init, socketid);
        }
    }
}
